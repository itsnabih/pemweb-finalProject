...
<div class="product-grid">
    ...
    @else @foreach ($produk as $item)
    <div class="product-card">
        <img
            src="{{ Storage::url($item->fotoProduk) }}"
            alt="{{ $item->namaProduk }}"
            class="product-image"
        />
        <h2 class="product-title">{{ $item->namaProduk }}</h2>
        <div class="stock-status">
            @if ($item->stokProduk <= 0) Stock Habis @else Stok:
...
<div class="add-product">
    <a href="{{ route('produk.create') }}" class="add-product-btn"
        >Tambah Barang</a
    >
</div>
